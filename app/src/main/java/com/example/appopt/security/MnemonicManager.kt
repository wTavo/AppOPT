...
/**
 * Gestor de frases mnemónicas de recuperación basado en el estándar criptográfico **BIP-39**.
 *
 * Características de seguridad y diseño:
 * - Genera frases de 12 palabras con 128 bits de entropía pura mediante [SecureRandom].
 * - Lista canónica estándar BIP-39 de 2048 palabras en inglés para máxima interoperabilidad y robustez.
 * - Validación estricta de palabras y conteo de términos para evitar errores tipográficos en la recuperación.
 * - Implementación pura en Kotlin con CERO dependencias de librerías externas.
 */
object MnemonicManager {
    ...
    /**
     * Genera una nueva frase mnemónica aleatoria de 12 palabras con 128 bits de entropía criptográfica.
     *
     * @return Lista inmutable con las 12 palabras generadas.
     */
     ...
     * @return `true` si la frase contiene exactamente 12 palabras reconocidas, `false` en caso contrario.
     */
    fun isValid12WordPhrase(phrase: String): Boolean {
        val words = phrase.trim().lowercase().split("\\s+".toRegex()).filter { it.isNotBlank() }
        if (words.size != 12) return false
        val wordSet = BIP39_WORDLIST_SET
        return words.all { wordSet.contains(it) }
    }

    /**
     * Normaliza una frase ingresada por el usuario limpiando espacios adicionales y convirtiendo a minúsculas.
     *
     * @param phrase Frase en bruto.
    ...
    /**
     * Conjunto optimizado para búsquedas O(1) durante la validación de frases.
     */
    private val BIP39_WORDLIST_SET: Set<String> by lazy {
        BIP39_WORDLIST.toHashSet()
    }

    /**
     * Diccionario canónico de 2048 palabras del estándar internacional BIP-39.
     */
    val BIP39_WORDLIST: List<String> = listOf(
        ...
        "scrub", "sea", "search", "season", "seat", "second", "secret", "section", "security", "seduce",
        "see", "seed", "seek", "segment", "select", "sell", "seminar", "senior", "sense", "sentence",
        ...
        "spread", "spring", "spy", "squad", "square", "squish", "squirrel", "stable", "stadium", "staff",
        "stage", "stair", "stamp", "stand", "start", "state", "stay", "steak", "steel", "stem",
